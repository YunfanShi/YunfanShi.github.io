...
- BETA 1.7.1 browser-AI automation: live account-model discovery and switching for ChatGPT, Gemini, and DeepSeek; fixed DeepSeek virtualized-response detection, recent/selected conversation routing, heartbeat/watchdog progress; and reply handoff back to Portal.
...
- `public/downloads/jackyun-companion-v1.7.1.zip` for BETA unpacked developer-mode installation.
- `public/downloads/jackyun-companion-v1.1.0.crx` remains the latest Stable signed candidate for signature verification, Verified CRX Uploads, Linux, or managed enterprise environments. A 1.7.1 CRX requires the local-only signing key and is intentionally not fabricated in this repository.
