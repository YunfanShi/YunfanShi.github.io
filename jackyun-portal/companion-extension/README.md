...
- BETA 1.5.1 browser-AI automation: explicit Portal opt-in, account eligibility check, prompt submission, stable-response detection, and reply handoff back to Portal.
...
- `public/downloads/jackyun-companion-v1.5.1.zip` for BETA unpacked developer-mode installation.
- `public/downloads/jackyun-companion-v1.1.0.crx` remains the latest Stable signed candidate for signature verification, Verified CRX Uploads, Linux, or managed enterprise environments. A 1.5.1 CRX requires the local-only signing key and is intentionally not fabricated in this repository.
