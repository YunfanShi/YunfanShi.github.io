...
- BETA 1.7.0 browser-AI automation: live account-model discovery and switching for ChatGPT, Gemini, and DeepSeek; verified Gemini and Qwen composer support; fixed recent/selected conversation routing; heartbeat/watchdog progress; stable-response detection; and reply handoff back to Portal.
...
- `public/downloads/jackyun-companion-v1.7.0.zip` for BETA unpacked developer-mode installation.
- `public/downloads/jackyun-companion-v1.1.0.crx` remains the latest Stable signed candidate for signature verification, Verified CRX Uploads, Linux, or managed enterprise environments. A 1.7.0 CRX requires the local-only signing key and is intentionally not fabricated in this repository.
