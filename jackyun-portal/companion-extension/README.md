...
- BETA 1.6.0 browser-AI automation: verified ChatGPT, DeepSeek, and Qwen composer support; new/recent/selected conversation routing; local open-conversation discovery; live Portal handshake; stable-response detection; and reply handoff back to Portal.
...
- `public/downloads/jackyun-companion-v1.6.0.zip` for BETA unpacked developer-mode installation.
- `public/downloads/jackyun-companion-v1.1.0.crx` remains the latest Stable signed candidate for signature verification, Verified CRX Uploads, Linux, or managed enterprise environments. A 1.6.0 CRX requires the local-only signing key and is intentionally not fabricated in this repository.
