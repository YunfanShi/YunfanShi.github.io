...
- BETA 1.5.2 browser-AI automation: live Portal handshake, upgrade-safe device version sync, fresh content-script injection, resilient prompt submission, stable-response detection, and reply handoff back to Portal.
...
- `public/downloads/jackyun-companion-v1.5.2.zip` for BETA unpacked developer-mode installation.
- `public/downloads/jackyun-companion-v1.1.0.crx` remains the latest Stable signed candidate for signature verification, Verified CRX Uploads, Linux, or managed enterprise environments. A 1.5.2 CRX requires the local-only signing key and is intentionally not fabricated in this repository.
