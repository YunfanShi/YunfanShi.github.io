...
export async function startUserChat(userId: string, subject: string, body: string): Promise<{ success: boolean; ticketId?: string; error?: string }> { const supabase = await requireAdminFeedback(); const { data, error } = await supabase.rpc('admin_start_user_chat', { p_user_id: userId, p_subject: subject.trim(), p_body: body.trim() }); if (!error) { revalidatePath('/admin/tickets'); revalidatePath('/admin/users'); } return error ? { success: false, error: error.message } : { success: true, ticketId: data as string }; }
...
export async function clearTicketLegacySync(id: string) { const supabase = await requireAdminFeedback(); const { data, error } = await supabase.rpc('admin_clear_ticket_legacy_sync', { p_report_id: id }); if (!error) revalidatePath('/admin/tickets'); return error ? { success: false, error: error.message } : { success: true, removed: Number(data ?? 0) }; }
export async function recordTicketOperation(id: string, operation: 'diagnostics_viewed') { const supabase = await requireAdminFeedback(); const { error } = await supabase.rpc('admin_record_ticket_operation', { p_report_id: id, p_operation: operation }); return error ? { success: false, error: error.message } : { success: true }; }
export async function sendTicketPasswordReset(id: string) { const supabase = await requireAdminFeedback(); const { data: report, error: reportError } = await supabase.from('bug_reports').select('user_id').eq('id', id).maybeSingle(); if (reportError || !report) return { success: false, error: reportError?.message ?? '工单不存在' }; const { data: profile, error: profileError } = await supabase.from('profiles').select('email').eq('id', report.user_id).maybeSingle(); if (profileError || !profile?.email) return { success: false, error: profileError?.message ?? '该用户没有可用于重置密码的邮箱。' }; const siteUrl = process.env.NEXT_PUBLIC_SITE_URL ?? (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'http://localhost:3000'); const { error } = await supabase.auth.resetPasswordForEmail(profile.email, { redirectTo: `${siteUrl}/auth/callback?type=recovery` }); if (error) return { success: false, error: error.message }; await supabase.rpc('admin_record_ticket_operation', { p_report_id: id, p_operation: 'password_reset_email_sent', p_details: {} }); return { success: true }; }
