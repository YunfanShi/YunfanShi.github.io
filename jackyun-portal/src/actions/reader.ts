...
function featureAccess(
  ...
  return {
    ...
    allowed:
      row.enabled &&
      (!row.beta_only || betaActive) &&
      hasPlanAccess(plan, row.minimum_plan),
  };
}

async function loadFeatureAccess(): Promise<{
  snapshot: FeatureAccessSnapshot;
  admin: NonNullable<ReturnType<typeof createAdminClient>> | null;
}> {
  ...
  const [featureResult, entitlementResult, betaResult] = await Promise.all([
    admin
      .from("app_features")
      .select(
        "key, display_name, description, enabled, beta_only, minimum_plan",
      ),
    user
      ? admin
  ...
  const plan =
    entitlement && (!entitlement.expires_at || entitlement.expires_at > now)
      ? (entitlement.plan_code ?? "free")
      : "free";
  const betaActive =
    (betaResult.data as { status?: string } | null)?.status === "accepted";
  const features = Object.fromEntries(
    ((featureResult.data ?? []) as FeatureRow[]).map((row) => [
      row.key,
      featureAccess(row, plan, betaActive),
    ]),
  );
  return {
    snapshot: {
...
export async function getReaderBootstrap(): Promise<ReaderBootstrap> {
  ...
  if (!admin || !features.novel_store?.allowed)
  ...
  const [catalogResult, unlockedResult] = await Promise.all([
    ...
    userId
      ? admin
          .from("user_novel_entitlements")
          .select("novel_id")
          .eq("user_id", userId)
      : Promise.resolve({ data: [], error: null }),
  ]);
  const unlockedIds = new Set(
    (unlockedResult.data ?? []).map(
      (row: { novel_id: string }) => row.novel_id,
    ),
  );
  const catalog = await Promise.all(
    (catalogResult.data ?? []).map(async (row) => {
      const unlocked =
        hasPlanAccess(plan, row.minimum_plan) || unlockedIds.has(row.id);
      ...
      return {
        ...
        unlocked,
        downloadUrl: fileResult.data?.signedUrl ?? null,
        coverUrl: coverResult.data?.signedUrl ?? null,
...
export async function redeemCode(
  ...
  if (!/^[A-Z0-9]{6,32}$/u.test(normalized))
    return { success: false, error: "请输入 6–32 位兑换码。" };
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  ...
  const { data, error } = await admin.rpc("redeem_reward_code", {
    p_user_id: user.id,
    p_code_hash: hash,
  });
  if (error) {
    const key = Object.keys(redemptionErrors).find((item) =>
      error.message.includes(item),
    );
    return {
      success: false,
      error: key ? redemptionErrors[key] : "兑换失败，请稍后重试。",
    };
  }
  return { success: true, reward: data as Record<string, unknown> };
}

export async function getRedemptionHistory(): Promise<
  Array<{ id: string; reward: Record<string, unknown>; redeemedAt: string }>
> {
  const supabase = await createClient();
  const {
    data: { user },
  ...
  const { data } = await supabase
    ...
    .select("id, reward_snapshot, redeemed_at")
    .eq("user_id", user.id)
    .order("redeemed_at", { ascending: false })
    .limit(20);
  return (data ?? []).map((row) => ({
    id: row.id,
    reward: row.reward_snapshot as Record<string, unknown>,
