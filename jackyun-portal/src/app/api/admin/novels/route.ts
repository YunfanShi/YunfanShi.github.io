...
export async function POST(request: NextRequest) {
  ...
  if (files.length < 1 || files.length > 20 || covers.length > files.length || customTitle.length > 160 || (files.length > 1 && customTitle.length > 0) || author.length > 120 || description.length > 1000 || !['zh', 'en'].includes(language) || !isPlanCode(minimumPlan)) return NextResponse.json({ error: '请选择 1–20 本小说；批量上传时书名请留空，封面数量不能超过小说数量。' }, { status: 400 });
