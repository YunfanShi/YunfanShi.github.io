...
export async function GET(
  _request: Request,
  context: RouteContext<"/api/reader/novels/[id]">,
) {
  ...
  if (!admin)
    return NextResponse.json(
      { error: "Reader service is unavailable" },
      { status: 503 },
    );
  ...
  const [novelResult, entitlementResult, ownershipResult] = await Promise.all([
    admin
      .from("novel_catalog")
      .select("enabled, minimum_plan, chapters_ready, content_revision")
      .eq("id", id)
      .maybeSingle(),
    admin
  ...
  if (novelResult.error || !novelResult.data?.enabled)
    return NextResponse.json(
      { error: "小说不存在或未上架。" },
      { status: 404 },
    );
  ...
  const plan =
    entitlement && (!entitlement.expires_at || entitlement.expires_at > now)
      ...
      : "free";
  if (
    !hasPlanAccess(plan, novelResult.data.minimum_plan) &&
  ...
  if (!novelResult.data.chapters_ready)
    return NextResponse.json(
      { error: "这本书尚未生成托管章节。" },
      { status: 404 },
    );
  const { data, error } = await admin
    ...
    .order("chapter_index");
  if (error)
    return NextResponse.json({ error: "章节读取失败。" }, { status: 500 });
  return NextResponse.json(
    {
