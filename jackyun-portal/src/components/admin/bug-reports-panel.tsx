...
const ticketLabels: Record<BugReport['ticket_type'], string> = { bug: '问题反馈', suspension_appeal: '暂停申诉', deletion_recovery: '账号恢复' };
...
export default function BugReportsPanel({ reports, initialTicketId }: { reports: BugReport[]; initialTicketId?: string }) {
  ...
  const load = (id: string) => startTransition(async () => { try { setDetail(await getBugReportDetail(id)); } catch { setNotice('无法加载工单详情。'); } });
  ...
  const decideAppeal = (approved: boolean) => { if (!selected || selected.ticket_type === 'bug') return; const action = selected.ticket_type === 'suspension_appeal' ? '取消账户暂停' : '恢复注销账户'; const response = prompt(approved ? `将${action}。请输入给用户的处理说明：` : '请输入驳回原因和后续建议：'); if (!response?.trim()) return; startTransition(async () => { const result = await resolveAccountAppeal(selected.id, approved, response); if (!result.success) return setNotice(result.error ?? '申诉处理失败。'); updateItem({ status: 'closed' }); setNotice(approved ? `申诉已批准，已${action}。` : '申诉已驳回，处理说明已发送给用户。'); load(selected.id); }); };
  ...
  return (
    <div className="grid min-h-[680px] gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
      <aside className="overflow-hidden rounded-2xl border border-[#eaecf0] dark:border-white/10">
        <div className="space-y-2 border-b border-[#eaecf0] p-3 dark:border-white/10"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索工单" className="h-10 w-full rounded-lg border border-[#d0d5dd] bg-transparent px-3 text-sm" /><select value={filter} onChange={(event) => setFilter(event.target.value as typeof filter)} className="w-full rounded-lg border border-[#d0d5dd] bg-transparent px-2 py-2 text-xs"><option value="active">未结束工单</option>{statuses.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
        ...
        {selected ? <>
          <header className="flex flex-wrap items-start justify-between gap-3 border-b border-[#eaecf0] pb-4 dark:border-white/10">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {selected.ticket_type === 'bug' && <button type="button" disabled={pending || selected.status === 'closed'} onClick={close} className="rounded-lg bg-[#155eef] px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">结束工单</button>}
