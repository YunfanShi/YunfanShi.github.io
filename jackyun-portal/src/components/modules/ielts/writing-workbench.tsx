...
import {
  buildWritingReviewPrompt, countWords, diffWriting, parseWritingFeedback, readFirstValidJson, recurringRuleKeys, targetWords, taskLabel, updateErrorHistory, writeRedundantJson,
  type ErrorHistoryEntry, type ExternalResponseFormat, type ReviewMode, type WritingFeedback, type WritingTask,
...
export default function WritingWorkbench() {
  ...
  const [externalFormat, setExternalFormat] = useState<ExternalResponseFormat>('json');
  const [externalMode, setExternalMode] = useState<ReviewMode>('diagnose');
  ...
  useEffect(() => {
    let storedDraft: DraftState | null = null; let storedHistory: ErrorHistoryEntry[] = []; let storedLanguage: UiLanguage = 'en'; let storedLayout: WorkspaceLayout = 'split';
    try { storedDraft = readStoredDraft(); storedHistory = JSON.parse(localStorage.getItem(HISTORY_KEY) || '[]') as ErrorHistoryEntry[]; storedLanguage = localStorage.getItem(LANGUAGE_KEY) === 'zh' ? 'zh' : 'en'; storedLayout = localStorage.getItem(LAYOUT_KEY) === 'stacked' ? 'stacked' : 'split'; } catch { /* Start clean when all local copies are unavailable. */ }
    queueMicrotask(() => { if (storedDraft) setDraft(storedDraft); if (Array.isArray(storedHistory)) setHistory(storedHistory); setUiLanguage(storedLanguage); setWorkspaceLayout(storedLayout); setHydrated(true); });
  }, []);
  useEffect(() => {
    if (!hydrated) return;
    try {
  ...
  const externalPrompt = useMemo(() => buildWritingReviewPrompt({ ...draft, mode: externalMode, previousRuleKeys, outputLanguage: uiLanguage, responseFormat: externalFormat }), [draft, externalFormat, externalMode, previousRuleKeys, uiLanguage]);
  ...
  async function review(mode: ReviewMode) {
    ...
    try {
      const prompt = buildWritingReviewPrompt({ ...draft, originalEssay, mode, previousRuleKeys, outputLanguage: uiLanguage });
      ...
      setFeedback(result); setResolved([]); setPreviousRuleKeys(keys); setHistory((current) => updateErrorHistory(current, draft.essayId, keys)); setMessage(mode === 'upgrade' ? c.upgradeDone : c.feedbackDone);
    } catch (error) { setMessage(error instanceof Error ? error.message : c.failed); } finally { setLoadingMode(null); }
  }
  ...
  function importExternalReply() {
    ...
    if (externalFormat === 'markdown') { setExternalReport(externalReply.trim()); setMessage(c.markdownImported); return; }
    try {
      const result = parseWritingFeedback(externalReply);
      const keys = result.issues.map((issue) => issue.ruleKey);
      setFeedback(result); setResolved([]); setPreviousRuleKeys(keys); setHistory((current) => updateErrorHistory(current, draft.essayId, keys)); setExternalReport(''); setMessage(c.imported);
      if (!draft.originalEssay) commitDraft({ ...draft, originalEssay: draft.essay }, true);
    } catch { setMessage(c.invalidReply); }
  }
  const stage = !draft.originalEssay ? 0 : feedback?.readyForUpgrade ? 2 : 1;

  return <div className="relative mx-auto max-w-[1540px] space-y-5 text-[var(--foreground)]">
    ...
    <section className={`grid gap-5 ${workspaceLayout === 'split' ? 'xl:h-[calc(100dvh-14rem)] xl:min-h-[620px] xl:grid-cols-[minmax(0,1.3fr)_minmax(440px,1fr)] xl:overflow-hidden' : 'grid-cols-1'}`}><div className={`min-w-0 space-y-5 ${workspaceLayout === 'split' ? 'xl:overflow-y-auto xl:overscroll-contain xl:pr-1' : ''}`} data-scroll-region={workspaceLayout === 'split' ? true : undefined}>
      <article className="overflow-hidden rounded-3xl border border-[var(--card-border)] bg-[var(--card)] shadow-[0_14px_45px_rgba(15,23,42,.07)] dark:shadow-none"><div className="flex flex-col gap-4 border-b border-[var(--card-border)] bg-[linear-gradient(90deg,rgba(14,116,144,.06),transparent)] p-4 sm:px-6"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="flex flex-wrap gap-2" role="group" aria-label={c.taskGroup}>{(['task1-academic', 'task1-general', 'task2'] as WritingTask[]).map((task) => <button type="button" key={task} onClick={() => updateTask(task)} className={`min-h-11 rounded-xl px-3.5 text-sm font-semibold transition ${draft.task === task ? 'bg-[#0e7490] text-white shadow-md shadow-cyan-950/15' : 'border border-transparent bg-[var(--background)] text-[var(--muted-foreground)] hover:border-[#67b9cc]'}`}>{taskLabel(task)}</button>)}</div><div className="flex items-center gap-3 text-sm"><span className={`rounded-full px-3 py-1.5 font-bold ${words >= target ? 'bg-[#dcfce7] text-[#166534] dark:bg-[#143f2a] dark:text-[#86efac]' : 'bg-[var(--background)] text-[var(--muted-foreground)]'}`}>{words} / {target}+ {c.words}</span><span className={`flex items-center gap-1.5 text-xs font-semibold ${saveState === 'error' ? 'text-[#dc2626]' : 'text-[#16845b]'}`}><span className="material-icons-round text-base">{saveState === 'error' ? 'cloud_off' : 'verified'}</span>{saveState === 'error' ? c.saveError : c.saved}</span></div></div><div className="flex items-center gap-2"><span className="mr-1 text-xs font-bold uppercase tracking-[.08em] text-[var(--muted-foreground)]">{c.layout}</span><button type="button" aria-pressed={workspaceLayout === 'split'} onClick={() => changeLayout('split')} className={`flex min-h-10 items-center gap-2 rounded-xl px-3 text-xs font-bold ${workspaceLayout === 'split' ? 'bg-[#0e7490] text-white' : 'border border-[var(--card-border)] bg-[var(--background)] text-[var(--muted-foreground)]'}`}><span className="material-icons-round text-lg">view_sidebar</span>{c.split}</button><button type="button" aria-pressed={workspaceLayout === 'stacked'} onClick={() => changeLayout('stacked')} className={`flex min-h-10 items-center gap-2 rounded-xl px-3 text-xs font-bold ${workspaceLayout === 'stacked' ? 'bg-[#0e7490] text-white' : 'border border-[var(--card-border)] bg-[var(--background)] text-[var(--muted-foreground)]'}`}><span className="material-icons-round text-lg">view_agenda</span>{c.stacked}</button></div></div>
        <div className="space-y-5 p-4 sm:p-6"><label className="block"><span className="mb-2 block text-sm font-bold uppercase tracking-[.08em] text-[var(--muted-foreground)]">{c.question}</span><textarea value={draft.question} onChange={(event) => editField('question', event.target.value)} rows={3} placeholder={c.questionPlaceholder} className="w-full resize-y rounded-2xl border border-[var(--card-border)] bg-[var(--background)] px-4 py-3.5 text-base leading-7 outline-none transition focus:border-[#0e7490] focus:ring-4 focus:ring-[#0e7490]/10" /></label><label className="block"><span className="mb-2 flex items-center justify-between text-sm font-bold uppercase tracking-[.08em] text-[var(--muted-foreground)]"><span>{c.essay}</span>{draft.originalEssay && <button type="button" onClick={() => originalRef.current?.scrollIntoView({ behavior: 'smooth' })} className="normal-case font-semibold tracking-normal text-[#0e7490] hover:underline">{c.originalLink}</button>}</span><textarea value={draft.essay} onChange={(event) => editField('essay', event.target.value)} rows={20} spellCheck lang="en" placeholder={c.essayPlaceholder} className="min-h-[500px] w-full resize-y rounded-2xl border border-[var(--card-border)] bg-[var(--background)] px-5 py-5 font-serif text-[18px] leading-[2] outline-none transition focus:border-[#0e7490] focus:ring-4 focus:ring-[#0e7490]/10 sm:px-7" /></label></div></article>
    ...
    <aside className={`flex min-w-0 flex-col gap-5 ${workspaceLayout === 'split' ? 'xl:overflow-y-auto xl:overscroll-contain xl:pl-1' : ''}`} data-scroll-region={workspaceLayout === 'split' ? true : undefined}><article className="order-1 rounded-3xl border border-[var(--card-border)] bg-[var(--card)] p-5 shadow-[0_14px_45px_rgba(15,23,42,.07)] dark:shadow-none"><div className="mb-5 flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[#d97706]">{c.loop}</p><h2 className="mt-1.5 text-xl font-semibold">{c.round}</h2></div>{feedback && <span className="rounded-xl bg-[#fff1d6] px-3 py-1.5 text-sm font-bold text-[#92400e] dark:bg-[#4a2e0c] dark:text-[#fcd34d]">Band {feedback.bandEstimate}</span>}</div><ol className="relative mb-5 grid grid-cols-4 gap-1 text-center text-[11px] font-bold"><span className="absolute left-[12%] right-[12%] top-4 h-px bg-[var(--card-border)]" />{c.stages.map((label, index) => <li key={label} className="relative"><span className={`mx-auto grid h-8 w-8 place-items-center rounded-full border-2 ${index <= stage ? 'border-[#0e7490] bg-[#0e7490] text-white' : 'border-[var(--card-border)] bg-[var(--card)] text-[var(--muted-foreground)]'}`}>{index + 1}</span><span className={`mt-2 block ${index === stage ? 'text-[#0e7490]' : 'text-[var(--muted-foreground)]'}`}>{label}</span></li>)}</ol><div className="grid gap-2 sm:grid-cols-2"><button type="button" disabled={Boolean(loadingMode)} onClick={() => review(feedback ? 'recheck' : 'diagnose')} className="min-h-12 rounded-xl bg-[#0e7490] px-4 text-sm font-bold text-white shadow-md shadow-cyan-950/15 disabled:opacity-50">{loadingMode === 'diagnose' || loadingMode === 'recheck' ? c.analysing : feedback ? c.recheck : c.diagnose}</button><button type="button" disabled={Boolean(loadingMode) || !feedback} onClick={() => review('upgrade')} className="min-h-12 rounded-xl border-2 border-[#0e7490] px-4 text-sm font-bold text-[#0e7490] disabled:opacity-40 dark:text-[#67e8f9]">{loadingMode === 'upgrade' ? c.upgrading : c.upgrade}</button></div>{message && <p role="status" className="mt-4 rounded-xl border border-[var(--card-border)] bg-[var(--background)] p-3 text-sm leading-6 text-[var(--muted-foreground)]">{message} {/config|配置|API key/i.test(message) && <Link href="/settings" className="ml-1 font-bold text-[#0e7490] underline">{c.settings}</Link>}</p>}</article>

      {feedback ? <article className="max-h-[calc(100dvh-300px)] overflow-y-auto rounded-3xl border border-[var(--card-border)] bg-[var(--card)] p-5 shadow-[0_14px_45px_rgba(15,23,42,.07)] dark:shadow-none" data-scroll-region><p className="text-sm leading-6 text-[var(--muted-foreground)]">{feedback.summary}</p>{feedback.priorities.length > 0 && <div className="mt-4 rounded-2xl border border-[#a7e2ca] bg-[#effcf7] p-3.5 dark:border-[#24634d] dark:bg-[#123a2d]"><p className="text-xs font-bold uppercase tracking-[.12em] text-[#147352] dark:text-[#86efac]">{c.priority}</p><ul className="mt-2 space-y-1 text-sm">{feedback.priorities.map((priority) => <li key={priority}>• {priority}</li>)}</ul></div>}<div className="mt-4 space-y-3">{feedback.issues.map((issue) => <section key={issue.id} className={`rounded-2xl border p-3.5 transition ${resolved.includes(issue.id) ? 'border-[#8acbb3] bg-[#eefaf5] opacity-65 dark:bg-[#123a2d]' : 'border-[var(--card-border)]'}`}><div className="flex items-center gap-2"><span className={`h-2.5 w-2.5 rounded-full ${issue.severity === 'high' ? 'bg-[#dc2626]' : issue.severity === 'medium' ? 'bg-[#f59e0b]' : 'bg-[#0284c7]'}`} /><p className="text-[11px] font-bold uppercase tracking-[.08em]">{issue.category}</p><button type="button" onClick={() => setResolved((items) => items.includes(issue.id) ? items.filter((id) => id !== issue.id) : [...items, issue.id])} className="ml-auto text-xs font-bold text-[#0e7490]">{resolved.includes(issue.id) ? c.undo : c.resolved}</button></div><blockquote className="mt-3 border-l-2 border-[#38bdf8] pl-3 font-serif text-sm italic leading-6">“{issue.quote}”</blockquote><p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">{issue.explanation}</p><p className="mt-2 rounded-xl bg-[var(--background)] p-3 text-sm font-medium leading-6"><span className="font-bold text-[#0e7490]">{c.think}</span> {issue.selfRevisionPrompt}</p></section>)}</div>{feedback.upgrades.length > 0 && <div className="mt-5 space-y-3"><h3 className="font-semibold">{c.upgrades}</h3>{feedback.upgrades.map((upgrade, index) => <section key={`${upgrade.original}-${index}`} className="rounded-2xl border border-[var(--card-border)] p-3.5"><span className="rounded-full bg-[#fff1d6] px-2 py-1 text-[10px] font-bold uppercase text-[#92400e] dark:bg-[#4a2e0c] dark:text-[#fcd34d]">{upgrade.type}</span><p className="mt-2 text-sm line-through opacity-55">{upgrade.original}</p><p className="mt-1 text-sm font-semibold text-[#147352] dark:text-[#86efac]">{upgrade.suggestion}</p><p className="mt-1 text-xs leading-5 text-[var(--muted-foreground)]">{upgrade.why}</p></section>)}</div>}</article> : <article className="rounded-3xl border border-[#abd8e6] bg-[linear-gradient(145deg,#f0faff,#ffffff)] p-5 dark:border-[#24516a] dark:bg-[linear-gradient(145deg,#0b2639,#101820)]"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-[#0e7490] text-white shadow-lg"><span className="material-icons-round">psychology_alt</span></div><h2 className="mt-4 text-lg font-semibold">{c.emptyTitle}</h2><p className="mt-2 text-sm leading-6 text-[var(--muted-foreground)]">{c.emptyBody}</p></article>}
      ...
      <article className="overflow-hidden rounded-3xl border border-[#99d6df] bg-[var(--card)] shadow-[0_14px_45px_rgba(15,23,42,.07)] dark:border-[#245b65] dark:shadow-none"><div className="bg-[linear-gradient(120deg,#ecfeff,#f0f9ff)] p-5 dark:bg-[linear-gradient(120deg,#0b2f35,#102a43)]"><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#0891b2] text-white shadow-lg shadow-cyan-950/15"><span className="material-icons-round">hub</span></span><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#0e7490] dark:text-[#67e8f9]">{c.external}</p><h2 className="font-semibold">{c.externalTitle}</h2></div></div><p className="mt-3 text-sm leading-6 text-[var(--muted-foreground)]">{c.externalBody}</p></div><div className="space-y-4 p-5"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1 2xl:grid-cols-2"><label className="text-xs font-bold text-[var(--muted-foreground)]">{c.reviewMode}<select value={externalMode} onChange={(event) => setExternalMode(event.target.value as ReviewMode)} className="mt-1.5 min-h-11 w-full rounded-xl border border-[var(--card-border)] bg-[var(--background)] px-3 text-sm text-[var(--foreground)]"><option value="diagnose">{c.modes[0]}</option><option value="recheck">{c.modes[1]}</option><option value="upgrade">{c.modes[2]}</option></select></label><label className="text-xs font-bold text-[var(--muted-foreground)]">{c.responseFormat}<select value={externalFormat} onChange={(event) => setExternalFormat(event.target.value as ExternalResponseFormat)} className="mt-1.5 min-h-11 w-full rounded-xl border border-[var(--card-border)] bg-[var(--background)] px-3 text-sm text-[var(--foreground)]"><option value="json">{c.json}</option><option value="markdown">{c.markdown}</option></select></label></div><button type="button" onClick={copyExternalPrompt} className="min-h-12 w-full rounded-xl bg-[#0891b2] px-4 text-sm font-bold text-white shadow-md shadow-cyan-950/15"><span className="material-icons-round mr-2 align-middle text-lg">content_copy</span>{promptCopied ? c.copied : c.copyPrompt}</button><details className="rounded-xl border border-[var(--card-border)]"><summary className="cursor-pointer px-3 py-3 text-xs font-bold text-[var(--muted-foreground)]">{c.promptPreview}</summary><textarea readOnly value={externalPrompt} rows={9} onFocus={(event) => event.currentTarget.select()} className="w-full resize-y border-t border-[var(--card-border)] bg-[var(--background)] p-3 font-mono text-xs leading-5 outline-none" /></details><textarea value={externalReply} onChange={(event) => setExternalReply(event.target.value)} rows={8} placeholder={c.pasteReply} className="w-full resize-y rounded-xl border border-[var(--card-border)] bg-[var(--background)] p-3 font-mono text-xs leading-5 outline-none focus:border-[#0891b2] focus:ring-4 focus:ring-[#0891b2]/10" /><button type="button" onClick={importExternalReply} className="min-h-11 w-full rounded-xl border-2 border-[#0891b2] px-4 text-sm font-bold text-[#0e7490] dark:text-[#67e8f9]">{c.importReply}</button>{externalReport && <div className="rounded-xl border border-[var(--card-border)] bg-[var(--background)] p-3"><p className="mb-2 text-xs font-bold uppercase tracking-[.1em] text-[#0e7490]">{c.report}</p><pre className="max-h-72 overflow-auto whitespace-pre-wrap font-sans text-sm leading-6 text-[var(--foreground)]">{externalReport}</pre></div>}</div></article>
