...
import {
  buildWritingReviewPrompt, countWords, diffWriting, findQuotedTextRange, highlightQuotedText, parseWritingFeedback, readFirstValidJson, recurringRuleKeys, targetWords, taskLabel, updateErrorHistory, writeRedundantJson,
...
export default function WritingWorkbench() {
  ...
  function startTransferEssay() { const next = { ...defaultDraft, essayId: newEssayId(), task: draft.task, secondsLeft: draft.task === 'task2' ? 2400 : 1200 }; commitDraft(next, true); setFeedback(null); setFeedbackHistory([]); setPreviousRuleKeys([]); setResolved([]); setActiveIssueId(null); setMessage(c.transferStarted); }
