    ...
    <div className="page-enter mx-auto max-w-6xl">
      ...
      <section className="relative mt-6 overflow-hidden rounded-3xl border border-[#a9c7ff] bg-gradient-to-br from-[#eef4ff] via-white to-[#eefbf5] p-6 shadow-[0_18px_55px_rgba(37,88,211,0.12)] dark:border-[#3b5e9e] dark:from-[#172239] dark:via-[#1c2430] dark:to-[#162b28] sm:p-7">
        ...
        <div className="relative grid gap-6 lg:grid-cols-[minmax(0,1fr)_280px] lg:items-center">
          <div>
            <div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-[#2458d3] px-3 py-1 text-xs font-bold text-white">首选推荐 · JackYun 自研</span><span className="rounded-full border border-[#86a9e8] px-3 py-1 font-mono text-xs text-[#2458d3] dark:text-[#aecbfa]">Companion 1.3.0</span></div>
            <h2 className="mt-4 text-2xl font-bold tracking-[-0.04em] text-[#172033] dark:text-white">先装 JackYun Companion，再按需补充脚本</h2>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-[#53627a] dark:text-[#bdc7d8]">一套扩展包含默认开启的内置广告拦截与隐私保护、首次使用指引、学习时间、专注、多设备同步和 English SafeGuard。净网规则随扩展离线提供，不依赖 Google 或境外订阅。</p>
            <ul className="mt-4 grid gap-2 text-sm text-[#26344a] dark:text-[#e2e8f2] sm:grid-cols-2"><li>✓ 网络拦截 + 页面广告位清理</li><li>✓ 当前网站一键暂停与放行名单</li><li>✓ ZNotes 刷题与 BestExam PDF 整理</li><li>✓ 本地优先统计与清晰隐私边界</li></ul>
          </div>
          <div className="grid gap-2 rounded-2xl border border-white/70 bg-white/75 p-4 backdrop-blur dark:border-white/10 dark:bg-black/15">
            <a href="/downloads/jackyun-companion-v1.3.0.zip" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#2458d3] px-4 text-sm font-bold text-white hover:bg-[#1749bd]"><span className="material-icons-round">extension</span>下载 Companion 1.3</a>
            ...
            <p className="text-center text-[11px] leading-4 text-[var(--muted-foreground)]">1.3.0 为可加载已解压扩展的预览 ZIP；签名 1.1.0 仍保留供发布核验。</p>
          </div>
        </div>
      </section>
