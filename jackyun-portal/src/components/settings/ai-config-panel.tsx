    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {providerMode === 'browser' && betaActive && <div className="space-y-3 rounded-xl border border-[#7f56d9]/30 bg-[#7f56d9]/5 p-4">
        ...
        <div className="rounded-xl border border-[var(--card-border)] bg-[var(--card)] p-3">
          <label className="block text-sm font-semibold">对话上下文<select value={conversationMode} onChange={(event) => setConversationMode(event.target.value as BrowserAiConversationMode)} className={`${selectClass} mt-1`}><option value="new">每次新建对话</option><option value="jackyun">使用 JackYun AI 专属对话</option><option value="recent">继续最近打开的对话</option><option value="selected">选择指定对话继续</option></select></label>
          {conversationMode === 'jackyun' && <p className="mt-2 rounded-lg bg-[#7f56d9]/10 px-3 py-2 text-xs leading-5 text-[#6941c6] dark:text-[#c4b5fd]">首次会新建一个专属对话，之后 JackYun 会一直回到这条对话继续。</p>}
