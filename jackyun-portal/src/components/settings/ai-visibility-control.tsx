...
export default function AiVisibilityControl() {
  ...
  const update = (next: Customization) => {
    setValue(next);
    applyCustomization(next);
    setNotice('界面状态已应用到本机。');
  };
  return <div className="space-y-4"><p className="text-sm leading-6 text-[var(--muted-foreground)]">AI 只生成经过校验的本机界面状态，不会隐藏助手，也不会执行任意 HTML、CSS 或 JavaScript。</p><div><p className="text-sm font-medium">强调色</p><div className="mt-2 flex flex-wrap gap-2">{(['blue', 'green', 'purple', 'orange'] as const).map((accent) => <button key={accent} type="button" onClick={() => update({ ...value, accent })} aria-pressed={value.accent === accent} className={`h-9 rounded-lg border px-3 text-xs font-semibold capitalize ${value.accent === accent ? 'border-[var(--brand)] bg-[var(--brand)] text-white' : 'border-[var(--card-border)]'}`}>{accent}</button>)}</div></div><div className="flex items-center justify-between gap-4"><div><p className="text-sm font-medium">卡片轮廓</p><p className="mt-1 text-xs text-[var(--muted-foreground)]">调整主要卡片和面板的圆角状态。</p></div><select value={value.cornerStyle} onChange={(event) => update({ ...value, cornerStyle: event.target.value as CornerStyle })} className="rounded-lg border border-[var(--card-border)] bg-[var(--background)] p-2 text-sm"><option value="rounded">圆润</option><option value="soft">轻圆角</option></select></div><button type="button" onClick={() => update(defaults)} className="rounded-lg border border-[var(--card-border)] px-3 py-2 text-xs font-semibold">恢复默认微调</button>{notice && <p className="text-xs text-[var(--muted-foreground)]">{notice}</p>}</div>;
}
