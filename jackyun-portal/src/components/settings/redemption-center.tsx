...
function rewardDetails(reward: Record<string, unknown>) {
  if (reward.type === 'novel') {
    const novels = Array.isArray(reward.novels) ? reward.novels as Array<{ title?: unknown }> : [];
    return [{ label: '权益', value: `永久图书 · ${Number(reward.count ?? (novels.length || 1))} 本` }, { label: '已加入阅读器', value: novels.length ? novels.map((novel) => String(novel.title ?? '未命名小说')).join('、') : String(reward.title ?? '已领取小说') }];
  }
