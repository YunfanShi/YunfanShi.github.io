...
export default function SettingsContent(props: Props) {
  ...
  return <div className="page-enter mx-auto max-w-7xl">
    <div className="mb-6"><h1 className="text-3xl font-bold tracking-[-.04em] text-[var(--foreground)]">设置</h1><p className="mt-1 text-sm text-[var(--muted-foreground)]">账户、外观、导航、同步和隐私设置会保存到你的 JackYun 账号。</p></div>
    <div className="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="self-start rounded-2xl border border-[var(--card-border)] bg-[var(--card)] p-3 lg:sticky lg:top-0">
        ...
        <select value={active} onChange={(event) => choose(event.target.value as CategoryId)} className="mt-3 w-full rounded-xl border border-[var(--card-border)] bg-[var(--background)] p-3 lg:hidden">{visibleCategories.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select>
      ...
      <main className="min-w-0 space-y-5">
        ...
        {active === 'account' && <><Panel title="个人资料"><ProfileEditor initialName={props.displayName} initialAvatar={props.avatarUrl} userId={props.userId} /></Panel><Panel title="账户安全"><ChangePasswordPanel hasPassword={props.hasPassword} /><Link href="/reset-password" className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-[#1a73e8]"><span className="material-icons-round text-lg">email</span>通过邮件重置密码</Link></Panel></>}
        {active === 'appearance' && <><Panel title="外观" description="这些偏好会跨设备同步。"><FullscreenToggle initialEnabled={settings.appearance_preferences?.showFullscreen === true} /><CloudPreferencesPanel sectionKey="appearance_preferences" initialValue={{ theme: 'light', density: 'comfortable', reducedMotion: false, showFullscreen: false, ...(settings.appearance_preferences || {}) }} fields={[{ key: 'theme', label: '主题', description: '选择亮色、灰色或深色界面', type: 'select', options: [{ value: 'light', label: '亮色' }, { value: 'gray', label: '灰色' }, { value: 'dark', label: '深色' }] }, { key: 'density', label: '界面密度', description: '调整列表和卡片的间距', type: 'select', options: [{ value: 'comfortable', label: '舒适' }, { value: 'compact', label: '紧凑' }] }, { key: 'reducedMotion', label: '减少动态效果', description: '减少页面转场和悬浮动画', type: 'boolean' }]} /></Panel></>}
        ...
        {active === 'companion' && <Panel title="Companion 与同步" description="管理扩展、设备、有效时间统计和同步隐私。"><CompanionSettingsPanel initialPreferences={settings.companion_preferences || {}} devices={props.companionDevices} /></Panel>}
        ...
        {active === 'ai' && <><Panel title="AI 配置" description="Base URL 与模型跨设备同步；API Key 将迁移到加密凭据存储。"><AiConfigPanel initialBaseUrl={props.aiConfig.baseUrl} initialApiKey={props.aiConfig.apiKey} initialModel={props.aiConfig.model} /></Panel><Panel title="语音"><TtsConfigPanel /></Panel></>}
        {active === 'data' && <Panel title="数据与隐私" description="导出账户数据，检查同步范围和数据保留。"><ExportDataPanel /></Panel>}
        {active === 'advanced' && <><Panel title="高级设置"><CloudPreferencesPanel sectionKey="advanced_preferences" initialValue={{ diagnostics: false, betaFeatures: false, ...(settings.advanced_preferences || {}) }} fields={[{ key: 'diagnostics', label: '诊断信息', description: '发生错误时保存不含正文和密码的技术信息', type: 'boolean' }, { key: 'betaFeatures', label: '实验功能', description: '提前体验尚在测试的功能', type: 'boolean' }]} /></Panel><BugReportPanel /><LoggerViewerWrapper /></>}
        {active === 'about' && <><Panel title="关于 JackYun Portal"><div className="flex items-center gap-4"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-[#e8f0fe] font-bold text-[#1a73e8]">JY</div><div><strong>JackYun Portal</strong><p className="text-sm text-[var(--muted-foreground)]">版本 v{APP_VERSION}</p></div></div><Link href="/update" className="mt-5 inline-flex rounded-xl border border-[var(--card-border)] px-4 py-2 text-sm font-semibold">查看更新历史</Link></Panel><DeleteAccountPanel /></>}
      </main>
    </div>
  </div>;
}
