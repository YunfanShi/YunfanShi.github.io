...
const chineseNumber =
  "[0-9０-９零〇○一二三四五六七八九十百千万两壹贰叁肆伍陆柒捌玖拾佰仟]+";
const romanNumber = "[ivxlcdm]{1,12}";
const englishWordNumber =
...
const chineseChapter = new RegExp(
  `^(?:正文(?:卷)?\\s*)?第\\s*${chineseNumber}\\s*[章节卷部篇回集幕话夜日册辑季期]${titleSuffix}$`,
  "iu",
);
const compoundChineseChapter = new RegExp(
  `^(?:(?:第\\s*${chineseNumber}\\s*卷|卷\\s*${chineseNumber})\\s*)?(?:正文(?:卷)?\\s*)?第\\s*${chineseNumber}\\s*[章节回话幕]${titleSuffix}$`,
  "iu",
);
const chineseVolume = new RegExp(
  `^(?:卷|部|篇|册|辑|幕)\\s*${chineseNumber}${titleSuffix}$|^(?:上|中|下)[篇部卷册]${titleSuffix}$`,
  "iu",
);
const englishChapter = new RegExp(
  `^(?:chapter|chap\\.?|ch\\.?|book|part|volume|vol\\.?|section|act|scene|episode|story)\\s*(?:no\\.?|number|#)?\\s*${englishNumber}${englishTitleSuffix}$`,
  "iu",
);
const headingText = "\\S[^。！？.!?]{0,58}";
const arabicPunctuatedHeading = new RegExp(
  `^[0-9０-９]{1,4}\\s*[.．、,:：;；|｜_~—–-]\\s*${headingText}$`,
  "iu",
);
const symbolicPunctuatedHeading = new RegExp(
  `^(?:[零〇○一二三四五六七八九十百千万两壹贰叁肆伍陆柒捌玖拾佰仟]+|${romanNumber}|[甲乙丙丁戊己庚辛壬癸])\\s*[.．、,:：;；|｜_~—–-]\\s*${headingText}$`,
  "iu",
);
const bracketedNumberHeading = new RegExp(
  `^[（(【\\[]\\s*(?:${chineseNumber}|${romanNumber}|${englishWordNumber})\\s*[）)】\\]]\\s*(?:[.．、,:：;；|｜_~—–-]\\s*)?${headingText}$`,
  "iu",
);
const circledNumberHeading = new RegExp(
  `^[①②③④⑤⑥⑦⑧⑨⑩⑪⑫⑬⑭⑮⑯⑰⑱⑲⑳㊀㊁㊂㊃㊄㊅㊆㊇㊈㊉]\\s*${headingText}$`,
  "u",
);
const spacedNumberHeading = new RegExp(
  `^(?:[0-9０-９]{1,3}|${romanNumber}|${englishWordNumber})\\s+${headingText}$`,
  "iu",
);
const numberOnlyHeading = new RegExp(
  `^(?:[0-9０-９]{1,5}|${romanNumber}|${englishWordNumber})$`,
  "iu",
);
const specialHeading =
  /^(?:内容简介|简介|作者序|译者序|出版说明|人物介绍|作品相关|序章|序幕|序言|前言|楔子|引子|正文|终章|终幕|终曲|尾声|大结局|后记|后日谈|间章|幕间|番外(?:篇|章)?(?:\s*[0-9０-９一二三四五六七八九十]+)?|外传(?:\s*[0-9０-９一二三四五六七八九十]+)?|附录(?:\s*[0-9０-９一二三四五六七八九十]+)?|致谢|鸣谢|prologue|epilogue|preface|foreword|introduction|interlude|afterword|appendix|acknowledgements?)(?:(?:\s*[-—–:：.]\s*|\s+)[^。！？.!?]{1,70})?$/iu;
...
export function detectChapterHeading(line: string): string | null {
  const title = line
    ...
    .replace(/^[-=*]{3,}\s*/u, "")
    .replace(
      /^[-*+]\s+(?=(?:第|卷|部|篇|chapter|book|part|section|act|scene|prologue|epilogue))/iu,
      "",
    )
    .replace(/^[【\[「『](.*)[】\]」』]$/u, "$1")
    .trim();
  ...
  if (
    /^[0-9０-９]{3,}:[0-9０-９]{1,2}:[0-9０-９]{1,2}(?:[、，,\s]|$)/u.test(
      title,
    )
  )
    return null;
  ...
  if (
    chineseChapter.test(title) ||
    compoundChineseChapter.test(title) ||
    chineseVolume.test(title) ||
    englishChapter.test(title) ||
    specialHeading.test(title) ||
    arabicPunctuatedHeading.test(title) ||
    symbolicPunctuatedHeading.test(title) ||
    bracketedNumberHeading.test(title) ||
    circledNumberHeading.test(title) ||
    spacedNumberHeading.test(title) ||
...
function arabicHeadingNumber(title: string): number {
  ...
  return Number(
    digits.replace(/[０-９]/gu, (digit) =>
      String(digit.charCodeAt(0) - 0xff10),
    ),
  );
}

function longestIncreasingHeadingIndexes(
...
export function splitNovelIntoChapters(
  ...
  for (const line of text.split("\n")) {
    ...
    if (title)
      candidates.push({
        offset,
        contentStart:
          offset + line.length + (offset + line.length < text.length ? 1 : 0),
        title,
        family: chapterHeadingFamily(title),
      });
  ...
  const strongCount = candidates.filter(
    (candidate) => candidate.family === "strong",
  ).length;
  const looseCounts = new Map<HeadingFamily, number>();
  candidates.forEach((candidate) => {
    if (candidate.family !== "strong" && candidate.family !== "special")
      looseCounts.set(
        candidate.family,
        (looseCounts.get(candidate.family) ?? 0) + 1,
      );
  });
  const dominantLoose = Array.from(looseCounts.entries()).sort(
    (left, right) => right[1] - left[1],
  )[0];
  const arabicCandidates = candidates.filter(
    (candidate) => candidate.family === "arabic-punctuated",
  );
  const increasingArabicIndexes =
    longestIncreasingHeadingIndexes(arabicCandidates);
  let arabicIndex = -1;
  const matches = candidates.filter((candidate) => {
    ...
    if (dominantLoose && dominantLoose[1] >= 2) {
      ...
      if (candidate.family === "arabic-punctuated" && increasingArabicIndexes)
        return increasingArabicIndexes.has(arabicIndex);
      return true;
    }
    return true;
  });
  ...
  if (frontMatter)
    chapters.push({
      ...
      title: "卷首",
      content: frontMatter,
      characterCount: frontMatter.length,
    });
  ...
  return chapters
    .filter((chapter) => chapter.content || chapter.title)
    .map((chapter, index) => ({ ...chapter, index }));
}

export function calculateNovelProgress(
  chapterIndex: number,
  chapterProgress: number,
  chapterCount: number,
): number {
  ...
  const safeProgress = Number.isFinite(chapterProgress)
    ? Math.min(1, Math.max(0, chapterProgress))
    : 0;
  return Math.min(1, Math.max(0, (safeChapter + safeProgress) / chapterCount));
}

export function calculateNovelProgressByCharacters(
  chapterIndex: number,
  chapterProgress: number,
  ...
  const safeProgress = Number.isFinite(chapterProgress)
    ? Math.min(1, Math.max(0, chapterProgress))
    : 0;
  const total = chapterLengths.reduce(
    (sum, length) => sum + Math.max(0, length),
    0,
  );
  if (!total)
    return calculateNovelProgress(
      safeChapter,
      safeProgress,
      chapterLengths.length,
    );
  const completed = chapterLengths
    .slice(0, safeChapter)
    .reduce((sum, length) => sum + Math.max(0, length), 0);
  return Math.min(
    1,
    Math.max(
      0,
      (completed + Math.max(0, chapterLengths[safeChapter]) * safeProgress) /
        total,
    ),
  );
}
...
export function inferNovelTitle(fileName: string): string {
  return (
    fileName
      .replace(/\.(?:txt|text|md|markdown|html?|epub)$/iu, "")
      .replace(/[_-]+/gu, " ")
      .trim() || "未命名小说"
  );
}
