-- Catalog metadata synchronization, reusable tags, and administrator-managed chapters.
...
UPDATE public.novel_catalog SET cover_updated_at = updated_at
WHERE cover_path IS NOT NULL AND cover_updated_at IS NULL;

CREATE INDEX novel_catalog_category_idx ON public.novel_catalog(category);
CREATE INDEX novel_catalog_tags_gin_idx ON public.novel_catalog USING gin(tags);
CREATE UNIQUE INDEX novel_catalog_content_hash_uidx ON public.novel_catalog(content_hash) WHERE content_hash IS NOT NULL;
...
DECLARE v_tags text[];
BEGIN
  SELECT COALESCE(array_agg(value ORDER BY first_position), ARRAY[]::text[]) INTO v_tags
  FROM (
    SELECT first_position, value FROM (
    ...
    ) AS deduplicated ORDER BY first_position LIMIT 20
  ) AS normalized;

  INSERT INTO public.novel_tags(name) SELECT value FROM unnest(v_tags) AS value
...
DECLARE v_count integer; v_revision integer;
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM jsonb_array_elements(p_chapters) AS chapter
    WHERE char_length(btrim(COALESCE(chapter->>'title', ''))) NOT BETWEEN 1 AND 200 OR chapter->>'content' IS NULL
  ) THEN RAISE EXCEPTION 'INVALID_CHAPTER'; END IF;
  ...
  SELECT p_novel_id, item.ordinality - 1, btrim(item.chapter->>'title'), item.chapter->>'content', char_length(item.chapter->>'content')
  FROM jsonb_array_elements(p_chapters) WITH ORDINALITY AS item(chapter, ordinality);
  UPDATE public.novel_catalog SET chapters_ready = true, content_revision = content_revision + 1,
    content_updated_at = now(), updated_at = now() WHERE id = p_novel_id RETURNING content_revision INTO v_revision;
...
BEGIN
  ...
  SET tags = COALESCE((SELECT array_agg(input.value ORDER BY input.ordinality)
    FROM unnest(catalog.tags) WITH ORDINALITY AS input(value, ordinality)
    WHERE lower(btrim(input.value)) <> OLD.normalized_name), ARRAY[]::text[]), updated_at = now()
  WHERE EXISTS (SELECT 1 FROM unnest(catalog.tags) AS value WHERE lower(btrim(value)) = OLD.normalized_name);
  RETURN OLD;
END;
$$;

CREATE TRIGGER remove_deleted_novel_tag_from_catalog AFTER DELETE ON public.novel_tags
